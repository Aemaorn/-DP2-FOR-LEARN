...
/// <summary>
/// Contains all SQL query definitions for benchmarking.
/// Queries are ported from tuning/test.sql.
/// </summary>
public static class QueryDefinitions
{
    /// <summary>
    /// Query 1: User Context Query (~11ms)
    /// Fetches user info with employee details and delegatees.
    /// </summary>
    public const string UserContextQuery = """
    ...
    /// Counts Plans the user has access to.
    /// </summary>
    public const string PlanCountQuery = """
        ...
        WHERE NOT (p."IsDeleted")
          ...
          AND (
            (p."Status" IN ('DraftPlan', 'RejectPlan', 'EditPlan')
             AND p."CreatedBy" = ANY (@createdBys))
            OR
            (p."Status" IN ('WaitingApprovePlan', 'WaitingAcceptor') AND EXISTS (
                ...
                WHERE p."Id" = p0."PlanId"
                  AND p0."IsCurrent"
                  AND (p0."UserId" = ANY (@userIds)
                       OR (s."Id" IS NOT NULL AND s."SuUserId" = ANY (@userIds)))
    ...
    /// Counts PlanAnnouncements the user has access to.
    /// </summary>
    public const string PlanAnnouncementCountQuery = """
    ...
    /// Counts Procurements in PreProcurement step the user has access to.
    /// </summary>
    public const string ProcurementPreProcurementCountQuery = """
                ...
                WHERE NOT (p3."IsDeleted")
                  ...
                  AND EXISTS (
                      ...
                      WHERE NOT (p4."IsDeleted")
                        ...
                        AND (p4."UserId" = ANY (@userIds)
                             OR (s0."Id" IS NOT NULL AND s0."SuUserId" = ANY (@userIds)))
                  )
            )
            OR
            EXISTS (
                ...
                WHERE NOT (p6."IsDeleted")
                  ...
                  AND EXISTS (
                      ...
                      WHERE NOT (p7."IsDeleted")
                        ...
                        AND (p7."UserId" = ANY (@userIds)
                             OR (s1."Id" IS NOT NULL AND s1."SuUserId" = ANY (@userIds)))
                  )
            )
            OR
            EXISTS (
                ...
                WHERE NOT (p9."IsDeleted")
                  ...
                  AND EXISTS (
                      ...
                      WHERE NOT (p10."IsDeleted")
                        ...
                        AND (p10."UserId" = ANY (@userIds)
                             OR (s2."Id" IS NOT NULL AND s2."SuUserId" = ANY (@userIds)))
                  )
            )
            OR
            EXISTS (
                ...
                WHERE NOT (p12."IsDeleted")
                  ...
                  AND EXISTS (
                      ...
                      WHERE NOT (p13."IsDeleted")
                        ...
                        AND p13."IsCurrent"
                        AND (p13."UserId" = ANY (@userIds)
                             OR (s3."Id" IS NOT NULL AND s3."SuUserId" = ANY (@userIds)))
                  )
            )
            OR
            EXISTS (
                ...
                WHERE NOT (p14."IsDeleted")
                  ...
                  AND EXISTS (
                      ...
                      WHERE NOT (p15."IsDeleted")
                        ...
                        AND p15."IsCurrent"
                        AND (p15."UserId" = ANY (@userIds)
                             OR (s4."Id" IS NOT NULL AND s4."SuUserId" = ANY (@userIds)))
                  )
            )
          )
        """;
    ...
    /// Counts Pw119 records the user has access to.
    /// </summary>
    public const string Pw119CountQuery = """
    ...
    /// Counts P79Clause2 records the user has access to.
    /// </summary>
    public const string P79Clause2CountQuery = """
    ...
    /// Counts PPettyCash records the user has access to.
    /// </summary>
    public const string PPettyCashCountQuery = """
    ...
    /// Counts PPettyCashReimbursement records the user has access to.
    /// </summary>
    public const string PPettyCashReimbursementCountQuery = """
    ...
    /// Counts CmDeliveryAcceptancePeriod records the user has access to.
    /// </summary>
    public const string DeliveryAcceptancePeriodCountQuery = """
    ...
    /// Counts PExpenseDisbursement records the user can approve.
    /// </summary>
    public const string ExpenseDisbursementCountQuery = """

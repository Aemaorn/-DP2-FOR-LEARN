...
public record ExportExcelPw119Request(
    string? Keyword,
...
public class ExportExcelPw119Endpoint : Endpoint<ExportExcelPw119Request>
{
    private readonly Dp2DbContext dbContext;
    private readonly IOperationService operationService;
    private readonly ProcurementProgram procurementProgram = new();

    public ExportExcelPw119Endpoint(Dp2DbContext dbContext, IOperationService operationService)
    {
    ...
    public override void Configure()
    {
        this.Options(builder =>
            builder.WithTags("Pw119")
                   .ProducesProblem(StatusCodes.Status500InternalServerError));
        this.Get("pw119/export-excel");
    }

    public override async Task HandleAsync(ExportExcelPw119Request req, CancellationToken ct)
    {
        ...
        if (rowData.Any())
        {
            ...
            {
                var thaiDate = $"{r.Pw119Date.Day}/{r.Pw119Date.Month}/{r.Pw119Date.Year + 543}";
                ...
                excelDocument.RowStyled(
                    ...
                    (r.Pw119Number, cellStyle.center),
    ...
    private async Task<List<ExportExcelPw119Row>> GetRowsDataAsync(ExportExcelPw119Request req, CancellationToken ct)
    {
        IQueryable<Pw119> query;
        ...
        switch (req.WorkProcess)
        {
            case WorkProcess.InProcess:
                ...
                query = userContext != null
                    ...
                    : this.dbContext.Pw119s.AsNoTracking();
                break;

            case WorkProcess.Related:
                var relatedUserId = UserId.From(req.UserId);
                query = this.dbContext.Pw119s
                    .Where(x =>
                        ((x.Status != Pw119Status.Draft || x.Status != Pw119Status.Rejected) && x.AuditInfo.CreatedBy == req.UserId) ||
                        (x.Status == Pw119Status.WaitingApproval && x.Acceptors.Any(ac =>
            ...
            case WorkProcess.Completed:
                ...
                query = this.dbContext.Pw119s
                    .Where(x =>
                        ...
                        (x.Status == Pw119Status.Paid && x.Acceptors.Any(ac => ac.UserId == completedUserId)));
                break;

            default:
                query = this.dbContext.Pw119s.AsNoTracking();
                break;
        }
        ...
        query = query
            ...
            .WhereIfTrue(!string.IsNullOrWhiteSpace(req.Keyword), x => EF.Functions.ILike(x.Subject, $"%{req.Keyword}%") || EF.Functions.ILike((string)x.Pw119Number, $"%{req.Keyword}%"))
        ...
        query = query.OrderBy(o => o.Pw119Date);

        var pw119List = await query.ToListAsync(ct);

        var rows = pw119List
            .SelectMany(pw119 =>
                pw119.Vendors
                     .OrderBy(v => v.Sequence)
                     .Select(v => new ExportExcelPw119Row(
                         v.VendorName,
                         string.Join(", ", v.VendorParcels.OrderBy(p => p.Sequence).Select(p => p.Item)),
                         v.VendorParcels.Sum(p => p.TotalPriceVat),
                         pw119.Pw119Date,
                         pw119.Pw119Number.Value)))
            .ToList();
...
public record ExportExcelPw119Row(
    string VendorName,
    string Items,

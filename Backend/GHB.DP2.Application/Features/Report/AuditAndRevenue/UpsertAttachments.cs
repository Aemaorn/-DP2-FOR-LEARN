...
public class UpsertAttachmentsRequest : IHasAttachmentsWithId
{
    ...
    public Guid AuditAndRevenueId { get; init; }
...
public class UpsertAttachmentsEndpoint : AuditAndRevenueEndpoint<UpsertAttachmentsRequest, Results<Ok, NotFound<string>>>
{
    private readonly Dp2DbContext dbContext;

    public UpsertAttachmentsEndpoint(ILogger<UpdateReportAuditAndRevenueEndpoint> logger, Dp2DbContext dbContext, IFileServiceClient fileServiceClient, IOperationService operationService)
    ...
    {
        this.Put("report/audit-revenue/{auditAndRevenueId:guid}/attachment");
        this.Description(b => b
                              .WithTags("Report/AuditAndRevenue")
                              .WithName("UpdateReportAuditAndRevenueAttachment")
    ...
    protected override async ValueTask<Results<Ok, NotFound<string>>> HandleRequestAsync(UpsertAttachmentsRequest req, CancellationToken ct)
    {
        ...
        var auditAndRevenue = await this.dbContext.RpAuditAndRevenues
                                        .Include(a => a.Attachments)
                                        .SingleOrDefaultAsync(w => w.Id == RpAuditAndRevenueId.From(req.AuditAndRevenueId), ct);

        if (auditAndRevenue is null)
        {
            return TypedResults.NotFound("ไม่พบข้อมูลรายการจัดซื้อจัดจ้าง");
        }

        await this.UpsertAttachments(auditAndRevenue, req.Attachments);

        this.dbContext.RpAuditAndRevenues.Update(auditAndRevenue);

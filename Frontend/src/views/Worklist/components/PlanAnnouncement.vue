...
<template>
  <DataView :value="store.worklistRes.planAnnouncements?.page?.data" data-key="id">
    <template #list="{ items }">
      <div v-for="(data, index) in items" :key="index" class="border-1 border-gray-300 rounded-sm mb-2 p-1">
        <div class="grid grid-cols-12 px-2">
          <div class="lg:col-span-8">
            <InfoRow label="เลขที่">
              <button class="cursor-pointer" type="button" @click="() => onNavigate(data.id)">
          ...
          <div
            ...
            <div class="mr-2 flex gap-2">
              ...
              <div class="flex flex-col gap-1 min-w-40">
                <BadgeStatus class="!w-full text-sm!" text-color-class="text-[#1D4ED8]" label="ขออนุมัติเผยแพร่จัดซื้อจัดจ้าง" />
                <BadgeStatus class="!w-full text-lg! leading-5!"
                  :color="getStatusColor(data.status)"
                  :label="WorklistAnnouncementStatusName(data.status)" />
              </div>
            </div>
  ...
  <Pagination :page-number="store.criteria.pageNumber" :page-size="store.criteria.pageSize"
    :total-record="store.worklistRes.planAnnouncements?.page?.totalRecords" @change="store.onChangePageSizeAsync" />
</template>

...
<template>
  <DataView :value="store.worklistRes.plans?.page?.data" data-key="id">
    <template #list="{ items }">
          ...
          <div
            ...
            <div class="mr-2 flex gap-2">
              ...
              <div class="flex flex-col gap-1 min-w-40">
                <BadgeStatus class="!w-full text-sm!" text-color-class="text-[#1D4ED8]" label="รายการจัดซื้อจัดจ้าง" />
                <BadgeStatus class="!w-full text-lg! leading-5!"
                  :color="getStatusColor(data.status)"
                  :label="WorkListPlanStatusName(data.status)" />
              </div>
            </div>
  ...
  <Pagination :page-number="store.criteria.pageNumber" :page-size="store.criteria.pageSize"
    :total-record="store.worklistRes.plans?.page?.totalRecords" @change="store.onChangePageSizeAsync" />
</template>

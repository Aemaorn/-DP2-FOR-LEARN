...
import Cam01AdjustContractConstants from '@/constants/CAM/CAM01/cam01.adjustContract';
...
const { Cam01AdjustContractBadgeStatus } = Cam01AdjustContractConstants;
...
const canEditDocument = computed(() => {
  return (
    store.body.status === Cam01AdjustContractStatus.Draft ||
    store.body.status === Cam01AdjustContractStatus.Rejected ||
    store.body.status === Cam01AdjustContractStatus.Edit
  );
});
...
const AdjustContractForm = defineAsyncComponent(
  (): Promise<typeof import('@/views/CAM/CAM01/components/AdjustContract/Sub/AdjustContractForm.vue')> =>
    import('@/views/CAM/CAM01/components/AdjustContract/Sub/AdjustContractForm.vue')
);
...
onMounted(async (): Promise<void> => {
  ...
  if (amendmentStore.body.id) {
    await onGetByIdAsync(amendmentStore.body.id, amendmentStore.body.adjustContractDuration?.id);
  }
});
</script>
...
<template>
  <TitleHeader label="ข้อมูลบันทึกต่อท้ายสัญญา">
    <template #action>
      <BadgeStatus :color="Cam01AdjustContractBadgeStatus(body.status).color"
        :label="Cam01AdjustContractBadgeStatus(body.status).label" v-if="body.status" />
    ...
    <div class="grid lg:grid-cols-7 gap-2">
      <div class="lg:col-span-5 lg:order-1 order-2">
        <Tabs v-model:value="currentTab" unstyled>
          ...
          <TabPanels>
            <TabPanel value="0">
              <AdjustContractForm v-model="body" />
            </TabPanel>
            ...
            <TabPanel value="1">
              <ContractAddendumDocument :docId="store.body.contractAddendumDocumentId"
                ...
                @restore-version="() => store.resetDocumentAsync('ExtendChange')" />
            </TabPanel>
      ...
      <div class="relative lg:col-span-2 lg:order-2 order-1">
        <div class="flex flex-col gap-4 ml-3 sticky top-[100px]">
          <div id="button-section" class="flex items-center justify-end gap-2" v-if="menuStore.hasManage">
            ...
            <div id="approver-section" class="flex items-center gap-2" v-if="isCanApprover">
              <ButtonRecall v-if="isCanReCall" @click="onRecallAsync" />
              <ButtonSendEdit @click="onRejectedAsync()" />
              <ButtonApprove v-if="!isLastApprover" @click="onAcceptorApprovedAsync()" />
              <ButtonConfirm v-if="isLastApprover" @click="onAcceptorApprovedAsync()" />
            </div>
          </div>
            ...
            <AccordionPanel value="999" class="mb-4" v-if="currentTab !== '0'">
              ...
              <AccordionContent>
                ...
                <Card class="rounded-none overflow-auto h-[800px]">
                  <template #content>
                    <!-- Dictionary mapping for adjust contract duration document. -->
                    <DocumentMapping pathToGet="contract-amendment/adjust-contract-duration" @on-click-select="onClickSelectMapping" />
                  </template>
                </Card>
              </AccordionContent>
            </AccordionPanel>
  ...
  <DialogReviewDocument v-if="!!store.body.id" v-model="showReviewDocumentDialog" :docId="reviwDocumentId"
    :docName="`adjust-contract-${new Date().toISOString()}-${store.body.id?.toString()}`"

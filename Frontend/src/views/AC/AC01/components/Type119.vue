<script setup lang="ts">
import { TitleHeader, InfoItem } from '@/components/cosmetic';
import { PaymentDetail, ParcelList, Disbursement } from './Part';
import { useAc01DetailStore } from '@/stores/AC/ac01';
import { computed } from 'vue';
import type { SourceDataW119 } from '@/models/ACC/acc001';

const store = useAc01DetailStore();

const sourceData = computed(() => store.body.source.data as SourceDataW119)
</script>

<template>
  <Card class="mb-4">
    <template #content>
      <TitleHeader label="รายละเอียด ว119" />
      <div class="px-4 mt-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <InfoItem title="ปีงบประมาณ" :content="sourceData.budgetYear" />
        <InfoItem title="วิธีจัดหา" :content="sourceData.supplyMethod" />
        <InfoItem title="" :content="sourceData.supplyMethodSpecialType" />
        <InfoItem title="ความเป็นมา" :content="sourceData.source" class="col-span-1 md:col-span-4" />
        <p class="font-bold col-span-1 md:col-span-4 mt-4">
          ตาราง 1
          รายการเกี่ยวกับค่าใช้จ่ายในการบริหารงานของหน่วยงานของรัฐที่ต้องดำเนินการภายใต้พระราชบัญญัติการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ
          พ.ศ. 2560
        </p>
        <InfoItem title="ลำดับค่าใช้จ่าย" :content="sourceData.w119Categories" class="col-span-1 md:col-span-4" />
        <InfoItem title="เหตุผลและความจำเป็น" :content="sourceData.reason" class="col-span-1 md:col-span-4" />
      </div>
    </template>
  </Card>
  <PaymentDetail />
  <ParcelList />
  <Disbursement v-if="store.state.isEditDisbursement"/>
</template>
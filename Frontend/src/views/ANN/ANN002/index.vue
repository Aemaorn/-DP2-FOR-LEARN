...
import { useAnnouncementReportListStore } from '@/stores/ANN/ANN002';
...
onMounted(async (): Promise<void> => {
  await Promise.all([
    listStore.onGetReportTypeOptionsAsync(),
    ...
    <TitleHeader label="รายงาน">
      <template #action>
        ...
        <Button
          ...
          severity="primary"
          variant="outlined"
          @click="router.push({ name: 'ann002Detail' })"
        />
      </template>
    </TitleHeader>
        ...
        <Form @submit="listStore.onGetListAsync">
          <div class="mt-10 space-y-8 lg:space-y-10">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-2">
              <Select
                label="ประเภทรายงาน"
                v-model="listStore.criteria.announcementReportTypeCode"
    ...
    <Card>
      <template #content>
        ...
        >
          ...
          >
            <template #header>
              <p class="w-full font-bold text-center whitespace-nowrap">ประเภทรายงาน</p>
            </template>
            <template #body="{ data }">
              <p>{{ data.announcementReportType ?? '-' }}</p>
            </template>
          </Column>
          ...
          >
            <template #header>
              <p class="w-full font-bold text-center whitespace-nowrap">ปี</p>
            </template>
            <template #body="{ data }">
          ...
          >
            ...
            <template #body="{ data }">
              ...
              >{{ data.documentName ?? `${data.announcementReportType} ${data.year}` }}</a>
          ...
          >
            <template #header>
              <p class="w-full font-bold text-center whitespace-nowrap">รายละเอียด</p>
            </template>
            <template #body="{ data }">
              <p>{{ data.discretion ?? '-' }}</p>
            </template>
            ...
            <template #body="{ data }">
              <div class="flex justify-center">
                <Button
                  ...
                  text
                  rounded
                  @click="router.push({ name: 'ann002Detail', params: { id: data.id } })"
                />
                <Button
                  icon="pi pi-trash"

...
watch(() => store.body.budgets, () => {
  store.body.budgets?.forEach((budgetGroup, groupIndex) => {
    budgetGroup.budgetAmount = getBudgetGroupAmount(groupIndex);
  });
...
<template>
  <Card class="mb-4">
    <template #content>
      ...
      <div class="my-4">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-2 mt-8 gap-y-8">
        ...
        <draggable v-model="store.body.budgets" handle=".drag-group" group="group" itemKey="sequence"
          ...
          <template #item="{ element: budgetGroup, index: groupIndex }: { element: Budget, index: number }">
            <div class="p-1 pt-10 bg-gray-100 mt-2 mb-4">
                ...
                <div class="mt-4">
                  <DataTable :value="store.body.budgets[groupIndex].details"
                    ...
                    <Column bodyStyle="vertical-align: top">
                      ...
                      <template #body="{ data }">
                        <InputNumber :min-fraction-digits="2" v-model="data.budget" rules="required" grouping :disabled="!store.status.canEdit || !menuStore.hasManage"/>
                      </template>
                    </Column>

...
const handleVersionRestored = async () => {
  await store.api.resetDocumentAsync('ApprovalContractDraft');
};
...
<template>
  <Card>
    <template #content>
      <TitleHeader label="เอกสารขออนุมัติสัญญา" />
      <ChEditor
        v-if="store.body.approvalContractDraftDocumentId"
        :docId="store.body.approvalContractDraftDocumentId"
        :docName="new Date().toISOString()"
        :readonly="props.readonly"
        ref="documentRef"
        :key="store.body.approvalContractDraftDocumentId"
        :versions="store.body.approvalContractDraftDocumentVersions ?? []"
        :canRestoreVersion="store.states.canRestoreApprovalContractDraftDocument"

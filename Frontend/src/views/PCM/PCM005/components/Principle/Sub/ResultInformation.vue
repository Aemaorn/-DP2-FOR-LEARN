  ...
  <DataTable :value="store.body.perfSupportDataDetails" tableStyle="min-width: 50rem">
    <ColumnGroup type="header">
      <Row>
        ...
        <div v-for="(value, index) in Array.from([2565, 2566])" :key="index">
          <Column :colspan="2">
            <template #header>
              <p class="text-center w-full font-bold">ม.ค. - ธ.ค. {{ value }}</p>
            </template>
          </Column>
        </div>
      </Row>
      <Row>
        <div v-for="(_, index) in Array.from([2565, 2566])" :key="index">
          <Column>
            <template #header>

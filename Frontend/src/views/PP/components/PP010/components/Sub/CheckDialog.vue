...
import type { QualificationResultDto, Shareholder } from '@/views/PP/models/PP006/pp006Model';
import type { TContractDraftBody } from '@/views/PP/models/PP0010/ContractDraft';
import type { TNationalityType } from '@/models/ST/st003';
...
const data = defineModel<TContractDraftBody>("vendor", {
  required: true,
});

const oldData = ref<TContractDraftBody>();

const checkTypeForTitle = computed((): string =>
  props.title === PP006EntrepreneurType.COI ? 'COI' : 'Watchlist'
);

const filteredShareholders = computed((): Shareholder[] =>
  data.value.shareholder?.filter((s): boolean => s.checkType == null || s.checkType === checkTypeForTitle.value) ?? []
);
...
const handleUpsert = () => {
  if (store.vendorId) {
    store.api.onUpsertAttachments(store.vendorId, type.value, filteredAttachments.value);
  }
};

const handleAdjustFileGroup = () => {
  const otherTypeAttachments =
    data.value.checkerAttachments?.map(a => ({
      ...a,
  ...
  data.value.checkerAttachments = [...otherTypeAttachments, ...newAttachments];
};
...
const onSubmitAsync = async (): Promise<void> => {
  ...
  if (props.title === PP006EntrepreneurType.Watchlist) {
    ...
    data.value.watchlistDate = new Date();
    filteredShareholders.value.forEach((s): void => {
...
const onClose = (): void => {
  show.value = false;
  data.value = { ...oldData.value } as TContractDraftBody;
};

const checkEntrepreneur = async (): Promise<void> => {
  const checkType = props.title === PP006EntrepreneurType.COI ? 'COI' : 'Watchlist';
  const isJuristic = data.value.detail.vendor?.type === 'JuristicPerson' || data.value.detail.vendor?.type === 'Consortium' || data.value.detail.vendor?.type === 'JointVenture';

  const item = ((): CheckHistorySuVendorItem | null => {
    const taxId = data.value.detail.vendor.taxpayerIdentificationNo || null;
    if (taxId) {
      return { taxpayerIdentificationNo: taxId, firstName: null, lastName: null, isJuristic };
    }
    const name = (data.value.detail.vendor.name ?? '').trim().replace(/\s+/g, ' ');
    if (!name) return null;
    if (!data.value.detail.vendor?.type || data.value.detail.vendor?.type === 'Individual') {
...
const SetCoiResultWhenUndefined = () => {
  if (props.title === PP006EntrepreneurType.COI && data.value.coiDate == undefined) {
...
const SetEGPResultWhenUndefined = () => {
  if (props.title === PP006EntrepreneurType.EGP && data.value.egpDate == undefined) {
...
const SetWatchlistResultWhenUndefined = () => {
  if (props.title === PP006EntrepreneurType.Watchlist && data.value.watchlistDate == undefined) {
...
const checkShareholders = async (): Promise<void> => {
  if (!filteredShareholders.value.length) return;
  ...
  const { data: results, status } = await entrepreneurCheckService.checkHistoryLookupAsync({
    vendorId: data.value.vendorId ?? null,
    checkType,
    items: filteredShareholders.value.map((s): CheckHistorySuVendorItem => {
...
watch(() => show.value, async (newValue) => {
  if (newValue) {
    ...
    data.value.shareholder = data.value.shareholder?.map((s): Shareholder => ({
      ...
      lastName: '',
      isJuristic: s.isJuristic ?? undefined,
    }));
  ...
  <Dialog v-model:visible="show" modal :style="{ width: '80vw' }" :draggable="false" :breakpoints="{ '575px': '90vw' }">
    <template #container>
      <Form @submit="onSubmitAsync()" class="p-5 overflow-auto" v-slot="{ handleSubmit }">
          ...
          <template #content>
            ...
            <div class="grid lg:grid-cols-3">
              <InfoItem title="สัญชาติของผู้ประกอบการ"
                :content="nationalityNameBycode(data.detail.vendor?.nationality as TNationalityType)" />
              <InfoItem v-if="data.detail.vendor?.type" title="ประเภท"
                :content="typeNameByCode(data.detail.vendor?.type)" />
            </div>
            <div class="grid lg:grid-cols-3 mt-3">
              <InfoItem title="ประเภทผู้ประกอบการ" :content="data.detail.vendor?.entrepreneurTypeName" />
              <InfoItem title="เลขประจำตัวผู้เสียภาษี" :content="data.detail.vendor?.taxpayerIdentificationNo" />
              <InfoItem title="ชื่อสถานประกอบการ" :content="data.detail.vendor?.registrationPlace" />
            </div>
            <div class="grid lg:grid-cols-3 mt-3">
              <InfoItem title="หมายเลขโทรศัพท์สำหรับติดต่อ" :content="data.detail.vendor?.tel" />
              <InfoItem title="อีเมล" :content="data.email" />
            </div>
          </template>
        </Card>
        <Card class="mt-5">
          <template #content>
            <TitleHeader
              :label="`ข้อมูล ${props.title == PP006EntrepreneurType.EGP ? 'ผู้ทิ้งงาน (e-GP)' : props.title}`">
              <template #action>
            ...
            <InputArea class="mt-4" label="หมายเหตุ" v-model="data.coiRemark"
              v-if="props.title === PP006EntrepreneurType.COI" :disabled="!menuStore.hasManage || disable" />
            <InputArea class="mt-4" label="หมายเหตุ" v-model="data.watchlistRemark"
              v-if="props.title === PP006EntrepreneurType.Watchlist" :disabled="!menuStore.hasManage || disable" />
            <InputArea class="mt-4" label="หมายเหตุ" v-model="data.egpRemark"
              v-if="props.title === PP006EntrepreneurType.EGP" :disabled="!menuStore.hasManage || disable" />
            <div class="flex justify-end">
              <div class="text-end">
                <p class="mt-5"
                  v-if="props.title === PP006EntrepreneurType.COI ? data.coiDate : (props.title === PP006EntrepreneurType.EGP ? data.egpDate : data.watchlistDate)">
                  ตรวจสอบ ณ วันที่ : {{ ToDateTime(props.title === PP006EntrepreneurType.COI ? data.coiDate :
                    (props.title ===
                      PP006EntrepreneurType.EGP ? data.egpDate : data.watchlistDate)) }}</p>
              </div>
            </div>
          </template>
        </Card>
                ...
                <tr v-for="value in filteredShareholders" :key="value.id" class="odd:bg-white even:bg-gray-50">
                  ...
                  <td class="border border-gray-300 px-3 py-2">
                    <div class="flex flex-wrap gap-4">
                      <div class="flex items-center gap-2">
                        <Checkbox v-model="value.isDirector" :binary="true" :inputId="`isDirector-pp010-${value.id}`" :disabled="disable || !menuStore.hasManage" />
                        <label :for="`isDirector-pp010-${value.id}`" class="cursor-pointer -mt-4">กรรมการ</label>
                      </div>
                      <div class="flex items-center gap-2">
                        <Checkbox v-model="value.isShareholder" :binary="true" :inputId="`isShareholder-pp010-${value.id}`" :disabled="disable || !menuStore.hasManage" />
                        <label :for="`isShareholder-pp010-${value.id}`" class="cursor-pointer -mt-4">ผู้ถือหุ้น</label>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="border border-gray-300 px-1 py-1">
                    <InputField v-model.trim="value.taxId" rules="digits13" eager :disabled="disable || !menuStore.hasManage" />
                  </td>
                  <td class="border border-gray-300 px-1 py-1">
        ...
        <div class="flex items-center gap-3 justify-end mt-5" v-if="menuStore.hasManage && !disable">

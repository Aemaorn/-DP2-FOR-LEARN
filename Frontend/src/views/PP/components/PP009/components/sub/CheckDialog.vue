...
const data = defineModel<VendorInfo>("vendor", {
  required: true,
});

const oldData = ref<VendorInfo>();
...
const handleUpsert = () => {
  if (data.value.id) {
    store.fn.onUpsertAttachments(data.value.id, type.value, filteredAttachments.value);
  }
};
...
const onSubmitAsync = async (): Promise<void> => {
  ...
  store.fn.syncCheckDataByTaxId(data.value);
...
const onClose = (): void => {
  show.value = false;
  data.value = { ...oldData.value } as VendorInfo;
};

const checkEntrepreneur = async (): Promise<void> => {
  const checkType = props.title === PP006EntrepreneurType.COI ? 'COI' : 'Watchlist';
  const isJuristic = data.value.entrepreneur?.type === 'JuristicPerson' || data.value.entrepreneur?.type === 'Consortium' || data.value.entrepreneur?.type === 'JointVenture';

  const item = ((): CheckHistorySuVendorItem | null => {
    const taxId = data.value.entrepreneur?.taxpayerIdentificationNo || null;
    if (taxId) {
      return { taxpayerIdentificationNo: taxId, firstName: null, lastName: null, isJuristic };
    }
    const name = (data.value.entrepreneur?.establishmentName ?? '').trim().replace(/\s+/g, ' ');
    if (!name) return null;
    if (!data.value.entrepreneur?.type || data.value.entrepreneur?.type === 'Individual') {
  ...
  const { data: results, status } = await entrepreneurCheckService.checkHistoryLookupAsync({
    vendorId: data.value.entrepreneur?.id ?? null,
    checkType,
    items: [item],
  });
...
watch(() => show.value, async (newValue) => {
  if (newValue) {
    ...
    if (tasks.length && store.states.isEdit) {
        ...
        <Card>
          <template #content>
            ...
            <div class="grid lg:grid-cols-3">
              <InfoItem v-if="data.entrepreneur?.nationality" title="สัญชาติของผู้ประกอบการ"
                :content="nationalityNameBycode(data.entrepreneur?.nationality)" />
              <InfoItem v-if="data.entrepreneur?.type" title="ประเภท"
                :content="typeNameByCode(data.entrepreneur?.type)" />
            </div>
            <div class="grid lg:grid-cols-3 mt-3">
              <InfoItem title="ประเภทผู้ประกอบการ" :content="data.entrepreneur?.entrepreneurTypeName" />
              <InfoItem title="เลขประจำตัวผู้เสียภาษี" :content="data.entrepreneur?.taxpayerIdentificationNo" />
              <InfoItem title="ชื่อสถานประกอบการ" :content="data.entrepreneur?.establishmentName" />
            </div>
            <div class="grid lg:grid-cols-3 mt-3">
              <InfoItem title="หมายเลขโทรศัพท์สำหรับติดต่อ" :content="data.entrepreneur?.tel" />
              <InfoItem title="อีเมล" :content="data.entrepreneur?.email" />
            </div>
          </template>
        </Card>
        <Card class="mt-5">
                ...
                <tr v-for="value in filteredShareholders" :key="value.id" class="odd:bg-white even:bg-gray-50">
                  ...
                  <td class="border border-gray-300 px-3 py-2">
                    <div class="flex flex-wrap gap-4">
                      <div class="flex items-center gap-2">
                        <Checkbox v-model="value.isDirector" :binary="true" :inputId="`isDirector-pp009-${value.id}`" :disabled="disable || !menuStore.hasManage" />
                        <label :for="`isDirector-pp009-${value.id}`" class="cursor-pointer -mt-4">กรรมการ</label>
                      </div>
                      <div class="flex items-center gap-2">
                        <Checkbox v-model="value.isShareholder" :binary="true" :inputId="`isShareholder-pp009-${value.id}`" :disabled="disable || !menuStore.hasManage" />
                        <label :for="`isShareholder-pp009-${value.id}`" class="cursor-pointer -mt-4">ผู้ถือหุ้น</label>
                      </div>
                    </div>
                  </td>

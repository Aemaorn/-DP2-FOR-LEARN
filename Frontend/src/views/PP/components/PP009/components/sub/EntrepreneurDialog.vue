...
const pp009Vendor = ref<VendorInfo>({
...
const getEntrepreneur = (): void => {
  const findData = store.body.vendors[props.vendorSelected!];

  if (findData) {
    const copyData: VendorInfo = JSON.parse(JSON.stringify(findData));
...
const onSubmitAsync = async (): Promise<void> => {
  store.body.vendors[props.vendorSelected!] = pp009Vendor.value
...
watch(() => show.value, (newValue) => {
  if (newValue && props.vendorSelected !== undefined) {
      ...
      <Form @submit="onSubmitAsync" @invalidSubmit="ToastHelper.invalidMessageToast()" class="p-5 overflow-auto">
        ...
        <Card>
          <template #content>
            ...
            <div v-if="pp009Vendor.purchaseOrderApprovalContractId">
              <div class="grid lg:grid-cols-3">
                <InfoItem title="สัญชาติของผู้ประกอบการ"
                  :content="nationalityNameBycode(pp009Vendor.entrepreneur!.nationality)" />
                <InfoItem title="ประเภท" :content="typeNameByCode(pp009Vendor.entrepreneur!.type)" />
              </div>
              <div class="grid lg:grid-cols-3 mt-3">
                <InfoItem title="ประเภทผู้ประกอบการ" :content="pp009Vendor.entrepreneur!.entrepreneurType" />
                <InfoItem title="เลขประจำตัวผู้เสียภาษี"
                  :content="pp009Vendor.entrepreneur!.taxpayerIdentificationNo" />
                <InfoItem title="ชื่อสถานประกอบการ" :content="pp009Vendor.entrepreneur!.establishmentName" />
              </div>
              <div class="grid lg:grid-cols-3 mt-3">
                <InfoItem title="หมายเลขโทรศัพท์สำหรับติดต่อ" :content="pp009Vendor.entrepreneur!.tel" />
                <InfoItem title="อีเมล" :content="pp009Vendor.entrepreneur!.email" />
              </div>
            </div>
          </template>
        </Card>
        <Card class="mt-5" v-if="pp009Vendor.purchaseOrderApprovalContractId">
          <template #content>
            <TitleHeader label="ข้อมูลสำหรับผู้ถือหุ้น">
              <template #action>
                <Button label="เพิ่มรายชื่อผู้ถือหุ้น" icon="pi pi-plus" severity="primary" variant="outlined"
                  ...
                  v-if="store.states.isEdit && menuStore.hasManage" />
              </template>
            </TitleHeader>
                    ...
                    <div class="flex gap-5">
                      ...
                      <div class="bg-white p-5 w-full">
                        <div class="grid lg:grid-cols-3 gap-2">
                          <InputField label="เลขประจำตัวผู้เสียภาษี" v-model.trim="data.taxId" rules="required"
                            :disabled="!store.states.isEdit || !menuStore.hasManage" />
                        </div>
                        <div class="grid lg:grid-cols-3 gap-2">
                          <InputField label="ชื่อ" v-model.trim="data.firstName" rules="required"
                            :disabled="!store.states.isEdit || !menuStore.hasManage" />
                          <InputField label="นามสกุล" v-model.trim="data.lastName" rules="required"
                            :disabled="!store.states.isEdit || !menuStore.hasManage" />
                        </div>
                      </div>
                      <div class="flex items-center gap-2" v-if="store.states.isEdit && menuStore.hasManage">
        ...
        <div class="mt-5 flex gap-2 justify-end items-center" v-if="store.states.isEdit && menuStore.hasManage">

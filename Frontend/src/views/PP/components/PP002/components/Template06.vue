...
const isBuyWithHireTemplate = computed(() => {
  const templateCode = store.PP002Detail.torDocumentTemplateCode;
  return ['TorBuyWithHire60', 'TorBuyWithHire80'].includes(templateCode);
});
...
const onInitTemplate = (): void => {
  const defaultRate = (isHireTemplate.value || value.value.supplyMethodTypeCode === SupplyMethodTypeConstant.Hire) ? 0.1 : 0.2;

  const defaultFineRates = (): FineRates[] => {
    if (value.value.fineRates && value.value.fineRates.length > 0) return value.value.fineRates;
    if (isBuyWithHireTemplate.value) return [{ sequence: 1, rate: 0.2 } as FineRates, { sequence: 2, rate: 0.1 } as FineRates];
    if (isHireTemplate.value) return [{ sequence: 1, rate: defaultRate } as FineRates, { sequence: 2, rate: defaultRate } as FineRates];
    return [{ sequence: 1, rate: defaultRate } as FineRates];
  };
  ...
  value.value = {
    ...
    paymentTerms: !value.value.paymentTerms || value.value.paymentTerms.length === 0 ? [{ isMA: false, proRateTypeCode: ProRateTypeCodeEnum.SplitPayment001, totalPeriodTypeCode: PeriodTypeCodeEnum.PeriodType004, details: [{ termNumber: 1 } as PaymentTermsDetail] } as PaymentTerms] : value.value.paymentTerms,
    warranties: value.value.warranties?.length === 0 ? [{} as Warranties] : value.value.warranties,
    fineRates: defaultFineRates(),
...
<template>
  ...
  <ParcelSection v-model="value.technicalSpecifications"
    :title-name="isHireTemplate ? 'คุณลักษณะเฉพาะของพัสดุ (งานจ้าง) / รายละเอียดของงาน (งานจ้างบำรุงรักษาฯ)' : 'คุณลักษณะเฉพาะของพัสดุ (งานซื้อ) / รายละเอียดของงาน (งานจ้างบำรุงรักษาฯ)'" />
  ...
  <BudgetProcurement v-model="value.budgets" title-name="วงเงินที่จะจัดซื้อจัดจ้าง" :disabled="!store.status.canEditTor"
    can-add-budget />
  <PhaseListSection v-model="value.paymentTerms" title-name="งวดงานและการจ่ายเงิน" :show-period-type="false"
    :show-payment-term-ma="true" v-if="value.paymentTerms" />

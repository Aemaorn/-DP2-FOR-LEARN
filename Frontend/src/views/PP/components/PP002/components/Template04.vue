...
import type { FineRates, PaymentTerms, PaymentTermsDetail, PP002Detail, TechnicalPeriods, TorCorrectiveMaintenanceModel, TorPreventiveMaintenanceModel, TorTrainingModel } from '@/views/PP/models/PP002/pp002Model';
import {
  ...
  FineSection,
  PhaseListSection,
} from '@/views/PP/components/PP002/components/sub/template04';
...
const onInitTemplate = (): void => {
  value.value = {
    ...value.value,
    technicalPeriods:
      value.value.technicalPeriods?.length === 0
        ? [{ periodTypeCode: PeriodTypeCodeEnum.Day } as TechnicalPeriods]
        : value.value.technicalPeriods,
    paymentTerms: !value.value.paymentTerms || value.value.paymentTerms.length === 0 ? [{ proRateTypeCode: ProRateTypeCodeEnum.SplitPayment003, isMA: false, totalPeriodTypeCode: PeriodTypeCodeEnum.PeriodType004, details: [{ termNumber: 1 } as PaymentTermsDetail] } as PaymentTerms] : value.value.paymentTerms,
...
<template>
  ...
  <DeliveryPeriodMaterialSection v-model="value.technicalPeriods[0]" title-name="ระยะเวลาดำเนินการ"
    label="ระยะเวลาการบำรุงรักษา" v-if="value.technicalPeriods" />

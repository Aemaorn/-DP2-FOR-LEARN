...
import {
  ...
  FineSection,
  WarrantySection,
} from '@/views/PP/components/PP002/components/sub/template01';
...
<template>
  ...
  <DeliveryPeriodMaterialSection v-model="value.technicalPeriods[0]" title-name="ระยะเวลา ส่งมอบพัสดุ"
    label="ผู้ที่ได้รับการคัดเลือกต้องส่งมอบภายใน" v-if="value.technicalPeriods" />
  <BudgetProcurement v-model="value.budgets" title-name="วงเงินที่จะจัดซื้อ" :disabled="!store.status.canEditTor" />
  ...
  <WarrantySection v-model="value.warranties[0]" title-name="การรับประกันความชำรุดบกพร่อง" v-if="value.warranties" />

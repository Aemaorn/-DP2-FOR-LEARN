...
import {
  ...
  FineSection,
  PhaseListSection,
} from '@/views/PP/components/PP002/components/sub/template10';
...
const onInitTemplate = (): void => {
  value.value = {
    ...value.value,
    technicalPeriods: value.value.technicalPeriods?.length === 0 ? [{ details: [] as TechnicalPeriodsDetail[], periodTypeCode: PeriodTypeCodeEnum.Day, } as TechnicalPeriods] : value.value.technicalPeriods,
...
<template>
  ...
  <ProcessingTimeSection v-model="value.technicalPeriods[0]" title-name="ระยะเวลาดำเนินการ"

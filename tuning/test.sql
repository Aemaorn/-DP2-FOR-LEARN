-- =====================================================
-- Test SQL Queries for Worklist API
-- Generated from raw_query.md
-- =====================================================
-- วิธีใช้: แก้ไขค่า parameters ด้านล่างให้ตรงกับข้อมูลจริงในระบบ
-- แล้ว run ทีละ query เพื่อทดสอบ
-- =====================================================

-- ===========================================
-- PARAMETERS - แก้ไขค่าเหล่านี้ก่อนทดสอบ
-- ===========================================
-- ตัวอย่าง: ค้นหา userId จากตาราง SuUser
-- SELECT "Id", "EmployeeCode" FROM "SystemUtility"."SuUser" LIMIT 10;

-- กำหนดค่า parameters (แก้ไข UUID ให้ตรงกับข้อมูลจริง)
DO $$
DECLARE
    v_user_id UUID := '00000000-0000-0000-0000-000000000001'::UUID;  -- แก้ไขเป็น userId จริง
    v_user_ids UUID[] := ARRAY['00000000-0000-0000-0000-000000000001']::UUID[];  -- แก้ไขเป็น userIds จริง
    v_created_bys UUID[] := ARRAY['00000000-0000-0000-0000-000000000001']::UUID[];  -- แก้ไขเป็น createdBys จริง
BEGIN
    RAISE NOTICE 'Test User ID: %', v_user_id;
    RAISE NOTICE 'Test User IDs: %', v_user_ids;
END $$;

-- ===========================================
-- Query 1: User Context Query (~11ms)
-- ===========================================
-- คำอธิบาย: ดึงข้อมูล user พร้อม employee info และ delegatee
...
FROM (
    ...
    WHERE s."Id" = '00000000-0000-0000-0000-000000000001'::UUID  -- แก้ไข userId
    LIMIT 1
) AS s4
LEFT JOIN (
...
-- ===========================================
-- คำอธิบาย: นับจำนวน Plan ที่ user มีสิทธิ์เข้าถึง
...
WHERE NOT (p."IsDeleted")
  ...
  AND (
    -- Creator conditions
    (p."Status" IN ('DraftPlan', 'RejectPlan', 'EditPlan')
     AND p."CreatedBy" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[]))  -- แก้ไข createdBys
    OR
    -- Approver conditions
    (p."Status" IN ('WaitingApprovePlan', 'WaitingAcceptor') AND EXISTS (
        ...
        WHERE p."Id" = p0."PlanId"
          AND p0."IsCurrent"
          AND (p0."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
               OR (s."Id" IS NOT NULL
                   AND s."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])))  -- แก้ไข userIds
    ...
    (p."Status" IN ('WaitingAssign', 'WaitingAnnouncement') AND (
        (SELECT p1."UserId"
         ...
         LIMIT 1) = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
    ))
    OR
    -- Assignee conditions
...
-- ===========================================
-- คำอธิบาย: นับจำนวน PlanAnnouncement ที่ user มีสิทธิ์เข้าถึง

SELECT count(*)::int AS plan_announcement_count
  ...
  AND (
    ...
    (p."Status" = 'WaitingAcceptor' AND EXISTS (
        ...
        WHERE p."Id" = p2."PlanAnnouncementId"
          AND p2."IsCurrent"
          AND (p2."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
               OR (s."Id" IS NOT NULL
                   AND s."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])))  -- แก้ไข userIds
    ))
  );
...
-- ===========================================
-- คำอธิบาย: นับจำนวน Procurement ใน Step PreProcurement ที่ user มีสิทธิ์
-- หมายเหตุ: Query นี้ใช้เวลานานมาก (~10 วินาที) เพราะมี EXISTS subqueries มากมาย

SELECT count(*)::int AS preprocurement_count
        ...
        WHERE NOT (p0."IsDeleted")
          ...
          AND EXISTS (
              ...
              WHERE p0."Id" = p1."PpAppointId"
                ...
                AND (p1."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
                     OR (s."Id" IS NOT NULL
                         AND s."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])))  -- แก้ไข userIds
          )
    )
        ...
        WHERE NOT (p3."IsDeleted")
          ...
          AND EXISTS (
              ...
              WHERE NOT (p4."IsDeleted")
                ...
                AND (p4."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
                     OR (s0."Id" IS NOT NULL
                         AND s0."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])))  -- แก้ไข userIds
          )
    )
    OR
        ...
        WHERE NOT (p6."IsDeleted")
          ...
          AND EXISTS (
              ...
              WHERE NOT (p7."IsDeleted")
                ...
                AND (p7."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
                     OR (s1."Id" IS NOT NULL
                         AND s1."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])))  -- แก้ไข userIds
          )
    )
    OR
        ...
        WHERE NOT (p9."IsDeleted")
          ...
          AND EXISTS (
              ...
              WHERE NOT (p10."IsDeleted")
                ...
                AND (p10."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
                     OR (s2."Id" IS NOT NULL
                         AND s2."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])))  -- แก้ไข userIds
          )
    )
    OR
        ...
        WHERE NOT (p12."IsDeleted")
          ...
          AND EXISTS (
              ...
              WHERE NOT (p13."IsDeleted")
                ...
                AND p13."IsCurrent"
                AND (p13."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
                     OR (s3."Id" IS NOT NULL
                         AND s3."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])))  -- แก้ไข userIds
          )
    )
    OR
        ...
        WHERE NOT (p14."IsDeleted")
          ...
          AND EXISTS (
              ...
              WHERE NOT (p15."IsDeleted")
                ...
                AND p15."IsCurrent"
                AND (p15."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
                     OR (s4."Id" IS NOT NULL
                         AND s4."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])))  -- แก้ไข userIds
          )
    )
  );
...
-- ===========================================
-- คำอธิบาย: นับจำนวน Pw119 ที่ user มีสิทธิ์เข้าถึง

SELECT count(*)::int AS pw119_count
FROM "Procurement"."Pw119" AS p
WHERE NOT (p."IsDeleted")
  AND (
    (p."Status" IN ('Draft', 'Rejected', 'Edit')
     AND p."CreatedBy" = '00000000-0000-0000-0000-000000000001'::UUID)  -- แก้ไข userId
    OR
...
-- ===========================================
-- คำอธิบาย: นับจำนวน P79Clause2 ที่ user มีสิทธิ์เข้าถึง

SELECT count(*)::int AS p79clause2_count
...
-- ===========================================
-- คำอธิบาย: นับจำนวน PPettyCash ที่ user มีสิทธิ์เข้าถึง

SELECT count(*)::int AS pettycash_count
...
-- ===========================================
-- คำอธิบาย: นับจำนวน PPettyCashReimbursement ที่ user มีสิทธิ์เข้าถึง

SELECT count(*)::int AS pettycash_reimbursement_count
...
-- ===========================================
-- คำอธิบาย: นับจำนวน CmDeliveryAcceptancePeriod ที่ user มีสิทธิ์เข้าถึง

SELECT count(*)::int AS delivery_acceptance_count
  ...
  AND (
        ...
        WHERE NOT (p1."IsDeleted")
          ...
          AND EXISTS (
              ...
              WHERE p1."Id" = p2."PJp005Id"
                AND p2."SuUserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
    ...
    (c."Status" IN ('Draft', 'Rejected') AND EXISTS (
        ...
        WHERE NOT (c7."IsDeleted")
          ...
          AND c7."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
    ))
    OR
    ...
    (c."Status" = 'WaitingCommitteeApproval' AND EXISTS (
        ...
        WHERE NOT (c8."IsDeleted")
          ...
          AND c8."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
    ...
    (c."Status" = 'WaitingAcceptance' AND EXISTS (
        ...
        WHERE NOT (c13."IsDeleted")
          ...
          AND c13."UserId" = ANY (ARRAY['00000000-0000-0000-0000-000000000001']::UUID[])  -- แก้ไข userIds
    ))
  );
...
-- ===========================================
-- คำอธิบาย: นับจำนวน PExpenseDisbursement ที่ user มีสิทธิ์อนุมัติ

SELECT count(*)::int AS expense_disbursement_count
...
-- ===========================================
-- HELPER QUERIES: หา userId และ userIds จริงจากระบบ
-- ===========================================

-- หา user ทั้งหมด
-- SELECT "Id", "EmployeeCode", "IsActive" FROM "SystemUtility"."SuUser" LIMIT 20;

-- หา user พร้อมชื่อ
-- SELECT su."Id", su."EmployeeCode", re."FirstName", re."LastName"
-- FROM "SystemUtility"."SuUser" su
-- INNER JOIN "Raws"."RawEmployee" re ON su."EmployeeCode" = re."Id"
-- LIMIT 20;

-- หา delegatee ของ user
-- SELECT sd."SuUserId", sd."DelegatorId", sdl."UserFullName"
-- FROM "SystemUtility"."SuDelegatee" sd
-- INNER JOIN "SystemUtility"."SuDelegator" sdl ON sd."DelegatorId" = sdl."Id"
-- WHERE sd."SuUserId" = '00000000-0000-0000-0000-000000000001';
